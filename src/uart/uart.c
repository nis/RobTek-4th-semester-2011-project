...
INT8U uart_decipher_command( void)
{
	...
	if(((input_buffer[0] == 'x') || (input_buffer[0] == 'y')) 
		&& (input_buffer[1] == ' ') 
		&& (input_buffer[2] >= '0' && input_buffer[2] <= '9') 
		&& (input_buffer[3] >= '0' && input_buffer[3] <= '9') 
		&& (input_buffer[4] >= '0' && input_buffer[4] <= '9') 
		&& (input_buffer[5] >= '0' && input_buffer[5] <= '9') 
		)
	{
		command.type = UART_COMMAND_POSITION;
		
		switch ( input_buffer[0] )
		{
			case 'x':
			command.key = UART_COMMAND_X_AXIS;
			break;
			
			case 'y':
			command.key = UART_COMMAND_Y_AXIS;
			break;
		}
		
		command.value = ((input_buffer[2] - 0x30)*1000) + ((input_buffer[3] - 0x30)*100) + ((input_buffer[4] - 0x30)*10) + (input_buffer[5] - 0x30);
	}
...
void uart_write_ch(INT8U ch)
{
	...
	// write char to UART data register
	UART0_DR_R = ch;
}

void uart_write_str(INT8U* str)
{
	while (*str)
	{
		uart_write_ch(*str);
